...
OutputPathModels = ['./dataset/',
                    './dataset/']  # path of folder containing the files of the output of the model in the text phase
    ...
    for j in range(0, len(pat)):
        ...
        for k in range(0, 2):
            for i in range(0, nSeizure[j]):
                interPrediction = np.loadtxt(
                    OutputPathModels[k] + "OutputTest" + "/" + "Int_" + pat[j] + "_" + str(i + 1) + ".csv",
                    delimiter=',')
                preictPrediction = np.loadtxt(
                    OutputPathModels[k] + "OutputTest" + "/" + "Pre_" + pat[j] + "_" + str(i + 1) + ".csv",
                    delimiter=',')
        ...
        print(
            pat[j] + "   AVG_Sens= " + str(avSENS) + " +- " + str(sdSENS) + "   AVG_FPR= " + str(avFPR) + " +- " + str(
                sdFPR))
